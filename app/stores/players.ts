...
export const usePlayerStore = defineStore('players', {
  ...
  actions: {
    async fetchAll(): Promise<void> {
      const api = useApi()
      this.players = await api<PlayerDto[]>('/api/Players')
    },

    async fetchById(id: string): Promise<void> {
      const api = useApi()
      this.selectedPlayer = await api<PlayerDto>(`/api/Players/${id}`)
    },
    ...
    async suspend(id: string): Promise<void> {
      const api = useApi()
      await api<void>(`/api/Admin/players/${id}/suspend`, { method: 'POST' })
      if (this.selectedPlayer?.id === id) {
    ...
    async activate(id: string): Promise<void> {
      const api = useApi()
      await api<void>(`/api/Admin/players/${id}/activate`, { method: 'POST' })
      if (this.selectedPlayer?.id === id) {
    ...
    async close(id: string): Promise<void> {
      const api = useApi()
      await api<void>(`/api/Admin/players/${id}/close`, { method: 'POST' })
      if (this.selectedPlayer?.id === id) {
    ...
    async verifyKyc(id: string): Promise<void> {
      const api = useApi()
      await api<void>(`/api/Admin/players/${id}/kyc`, { method: 'POST', body: { verified: true } })
      if (this.selectedPlayer?.id === id) {
    ...
    async fetchRiskProfile(id: string): Promise<void> {
      const api = useApi()
      this.riskProfile = await api<RiskProfileDto>(`/api/Compliance/players/${id}/risk`)
    },
  },
})
